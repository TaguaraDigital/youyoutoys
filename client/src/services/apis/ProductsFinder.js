...
ProductsFinder.create = async ({code, brand, category, description, inventory, price, pack}) => {
    const response = await fetch(URL, {
        method: "POST",
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ code, brand, category, description, inventory, price, pack })
    })
    return await response.json()
}
